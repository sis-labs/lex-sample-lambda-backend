...
def get_duration(appointment_type):
    # TODO: this should be fetch from a recommandation engine
    appointment_duration_map = {'general information': 30, 'contract information': 60, 'offer information': 30}
...
def build_options(slot, appointment_type, date, booking_map):
    ...
    if slot == 'AppointmentType':
        return [
            {'text': 'general information (30 min)', 'value': 'general information'},
            {'text': 'contract information (60 min)', 'value': 'contract information'},
            {'text': 'offer information (30 min)', 'value': 'offer information'}
        ]
    elif slot == 'Date':
...
def make_appointment(intent_request):
    ...
    first_name = intent_request['currentIntent']['slots']['FirstName']
    last_name = intent_request['currentIntent']['slots']['LastName']
    phone_number = intent_request['currentIntent']['slots']['PhoneNumber']
    mail_address = intent_request['currentIntent']['slots']['EmailAddress']
    ...
    if source == 'DialogCodeHook':
        ...
        if not appointment_type:
            logger.debug("Appointment type not found, send back question to gather this information")
            return elicit_slot(
                output_session_attributes,
                intent_request['currentIntent']['name'],
        ...
        if appointment_type and not date:
            logger.debug("Appointment type defined but no date found, sending back question to gather this information")
            return elicit_slot(
                output_session_attributes,
                intent_request['currentIntent']['name'],
                intent_request['currentIntent']['slots'],
        ...
        # TODO: the phone number, mail address and firstname or lastname are required items, provide implementation to check / gather about that,

        if appointment_type and date:
            logger.debug("Appointment request is correct, continue processing")
    ...
    return close(
        ...
        {
            'contentType': 'PlainText',
            'content': 'Okay, I have booked your appointment.  We will see you at {} on {}. You will receive a confirmation email soon. Thanks!'.format(build_time_output_string(appointment_time), date)
        }
    )
...
def lambda_handler(event, context):
    ...
    # Since we deployed that in virginia, by default, we treat 
    # the user request as coming from the America/New_York time zone.
    os.environ['TZ'] = 'America/New_York' # This should come from the env
    time.tzset()
